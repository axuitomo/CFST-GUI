...
type mobileTraceDiagnosticSample struct {
...
type mobileTraceDiagnostics struct {
	reasonCounts  map[string]int
	samples       []mobileTraceDiagnosticSample
...
func newMobileTraceDiagnostics(cfg probeConfig) *mobileTraceDiagnostics {
	return &mobileTraceDiagnostics{
		reasonCounts:  make(map[string]int),
		statusCounts:  make(map[string]int),
		traceColoMode: strings.TrimSpace(cfg.TraceColoMode),
		traceURL:      strings.TrimSpace(cfg.TraceURL),
	}
}

func (d *mobileTraceDiagnostics) Record(entry task.TraceDiagnostic) {
	if d == nil {
		return
	}
	...
	d.samples = append(d.samples, mobileTraceDiagnosticSample{
...
func (d *mobileTraceDiagnostics) Empty() bool {
	return d == nil || (len(d.reasonCounts) == 0 && len(d.samples) == 0 && len(d.statusCounts) == 0)
}

func (d *mobileTraceDiagnostics) Payload() map[string]any {
	if d.Empty() {
		return nil
	}
...
func (d *mobileTraceDiagnostics) Summary() string {
	if d.Empty() {
		return ""
	}
	...
	if reason != "" && count > 0 {
		parts = append(parts, fmt.Sprintf("%s %d 次", mobileTraceReasonLabel(reason), count))
	}
...
func (d *mobileTraceDiagnostics) topReason() (string, int) {
...
func (d *mobileTraceDiagnostics) topStatus() (string, int) {
...
func mobileTraceReasonLabel(reason string) string {
