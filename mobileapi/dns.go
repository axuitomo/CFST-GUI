...
func (s *Service) PushCloudflareDNSRecords(payloadJSON string) string {
		...
		for index, ip := range ips {
			...
			if index < len(existing) {
				if _, err := client.updateRecord(ctx, cfg, existing[index].ID, record); err != nil {
					return encodeCommand(commandResultFor("DNS_UPDATE_FAILED", nil, err.Error(), false, nil, warnings))
				}
				summary.Updated++
				continue
			}
			if _, err := client.createRecord(ctx, cfg, record); err != nil {
				return encodeCommand(commandResultFor("DNS_CREATE_FAILED", nil, err.Error(), false, nil, warnings))
			}
			summary.Created++
		}
		if len(existing) > len(ips) {
			for _, extra := range existing[len(ips):] {
				if err := client.deleteRecord(ctx, cfg, extra.ID); err != nil {
					return encodeCommand(commandResultFor("DNS_DELETE_FAILED", nil, err.Error(), false, nil, warnings))
				}
				summary.Deleted++
			}
		}
	...
	}, fmt.Sprintf("Cloudflare DNS 覆盖推送完成：创建 %d、更新 %d、删除 %d、忽略 %d。", summary.Created, summary.Updated, summary.Deleted, summary.Ignored), true, nil, warnings))
}
...
func (c *cloudflareDNSClient) do(ctx context.Context, method, endpoint string, body any, target any) error {
	...
	return cloudflareResponseError(target)
}
