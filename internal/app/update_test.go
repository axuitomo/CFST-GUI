...
func TestUpdateHTTPClientDoesNotUseEnvironmentProxy(t *testing.T) {
	transport, ok := httpClientForUpdates.Transport.(*http.Transport)
	if !ok {
		t.Fatalf("update transport = %T, want *http.Transport", httpClientForUpdates.Transport)
	}
	if transport.Proxy != nil {
		t.Fatal("update HTTP client should not use environment proxy")
	}
}
