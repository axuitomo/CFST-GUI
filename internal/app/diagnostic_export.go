...
func (a *App) ExportDiagnosticPackage(payload map[string]any) DesktopCommandResult {
	fileName := diagnosticPackageFileName(payload, "cfst-diagnostics.zip")
	body, included, err := a.buildDiagnosticPackage()
	if err != nil {
		return desktopCommandResult("DIAGNOSTIC_PACKAGE_BUILD_FAILED", nil, err.Error(), false, nil, nil)
	}
	data := map[string]any{
		"file_name":     fileName,
		"included":      included,
	...
	if targetURI := strings.TrimSpace(stringValue(firstNonNil(payload["target_uri"], payload["targetUri"], payload["uri"]), "")); targetURI != "" {
		data["content_base64"] = base64.StdEncoding.EncodeToString(body)
		data["target_uri"] = targetURI
		return desktopCommandResult("DIAGNOSTIC_PACKAGE_EXPORT_OK", data, "诊断包已准备导出。", true, nil, nil)
	}
	targetPath := strings.TrimSpace(stringValue(firstNonNil(payload["target_path"], payload["targetPath"], payload["path"]), ""))
	if targetPath == "" {
		targetPath = configuredDiagnosticPackageExportPath(payload, fileName)
	}
	if err := os.MkdirAll(filepath.Dir(targetPath), 0o755); err != nil {
		return desktopCommandResult("DIAGNOSTIC_PACKAGE_WRITE_FAILED", nil, err.Error(), false, nil, nil)
	}
	...
	return desktopCommandResult("DIAGNOSTIC_PACKAGE_EXPORT_OK", data, "诊断包已导出。", true, nil, nil)
}

func (a *App) buildDiagnosticPackage() ([]byte, []string, error) {
	buf := &bytes.Buffer{}
	...
	for _, item := range []struct {
		...
		{name: "logs/cfip-log.txt", path: debugLogFilePath()},
		{name: "logs/error-log.txt", path: errorLogFilePath()},
	} {
	...
	if err := addJSON("status/scheduler.json", a.currentSchedulerStatus()); err != nil {
		_ = archive.Close()
		return nil, nil, err
	}
	if err := addJSON("status/runtime.json", a.ensureRuntimeCleaner().Status()); err != nil {
		_ = archive.Close()
		return nil, nil, err
	}
	...
	if err := addJSON("config/config-summary.json", redactDiagnosticConfigSnapshot(snapshot)); err != nil {
		_ = archive.Close()
		return nil, nil, err
	}
	if err := addRecentTaskSnapshotsToZip(archive, &included, taskSnapshotsRootPath(), 20); err != nil {
		_ = archive.Close()
		return nil, nil, err
	}
...
func diagnosticPackageFileName(payload map[string]any, fallback string) string {
...
func configuredDiagnosticPackageExportPath(payload map[string]any, fileName string) string {
...
func redactDiagnosticConfigSnapshot(snapshot map[string]any) map[string]any {
	redacted := deepCloneMap(sanitizeDesktopConfigSnapshot(snapshot))
...
func addRecentTaskSnapshotsToZip(archive *zip.Writer, included *[]string, root string, limit int) error {
	...
	for _, entry := range entries {
		...
		if err := json.Unmarshal(raw, &snapshot); err != nil || !isTerminalTaskSnapshotStatus(snapshot.Status) {
			continue
		}
