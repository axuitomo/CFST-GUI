...
const settings = reactive<SettingsForm>({
  ...
  minDelayMs: 0,
  loggingEnabled: true,
  loggingDurability: "split",
  loggingLevel: "error",
  loggingMonitorEnabled: true,
...
let sourceLeaveSaveTarget: { view: ViewName } | null = null;
...
function frontendErrorMessage(error: unknown) {
  return error instanceof Error ? error.message : asString(error) || "Unknown frontend error";
}

function frontendErrorStack(error: unknown) {
  return error instanceof Error ? error.stack || "" : "";
}

async function reportFrontendRuntimeError(source: string, error: unknown, context: Record<string, unknown> = {}) {
  const payload = {
    ...context,
    message: frontendErrorMessage(error),
    source,
    stack: frontendErrorStack(error),
  };
  appendLog("frontend.runtime_error", payload);
  const result = await recordFrontendRuntimeError(payload);
  if (!result.ok) {
    appendLog("frontend.runtime_error.log_failed", result.message || result.code);
  }
}

function reportFrontendRuntimeErrorSilently(source: string, error: unknown, context: Record<string, unknown> = {}) {
  void reportFrontendRuntimeError(source, error, context);
}

function handleFrontendWindowError(event: ErrorEvent) {
  reportFrontendRuntimeErrorSilently("window.error", event.error || event.message, {
    colno: event.colno,
...
function handleFrontendUnhandledRejection(event: PromiseRejectionEvent) {
  reportFrontendRuntimeErrorSilently("window.unhandledrejection", event.reason);
}
...
function notifyActiveProbeBlocked(title: string) {
  ...
  void navigateTo({ view: "dashboard" });
}
...
function applyConfigSnapshot(snapshot: ConfigSnapshot) {
  ...
  settings.loggingEnabled = Boolean(normalized.logging.enabled);
  settings.loggingDurability = normalized.logging.durability || "split";
  settings.loggingLevel = normalized.logging.level || "error";
  settings.loggingMonitorEnabled = Boolean(normalized.logging.monitor_enabled);
  settings.loggingRetentionDays = asCount(normalized.logging.retention_days, 7);
...
function buildConfigSnapshot() {
  ...
  return {
    ...
    logging: {
      durability: settings.loggingDurability === "split" ? "split" : "split",
      enabled: settings.loggingEnabled,
      level: settings.loggingLevel === "debug" || settings.loggingLevel === "info" || settings.loggingLevel === "warn" ? settings.loggingLevel : "error",
      monitor_enabled: settings.loggingMonitorEnabled,
      retention_days: boundedCount(settings.loggingRetentionDays, 7, 1, 365),
    },
    probe: {
      concurrency: {
    ...
    scheduler: {
      ...
      interval_minutes: settings.schedulerTriggerMode === "interval" ? (settings.schedulerEnabled ? positiveCount(settings.schedulerIntervalMinutes, 60) : nonNegativeCount(settings.schedulerIntervalMinutes, 0)) : 0,
...
function applyNavigation(target: { view?: ViewName }) {
  if (target.view) {
    selectedView.value = target.view;
  }
}

function shouldSaveSourcesBeforeNavigation(target: { view: ViewName }) {
  if (selectedView.value !== "sources") {
    return false;
  }
  if (target.view === "sources") {
    return false;
  }
  return sourcePageHasUnsavedChanges();
}

async function navigateTo(target: { view?: ViewName }) {
  const nextTarget = {
  ...
  if (selectedView.value === "settings" && nextTarget.view !== "settings") {
...
async function exportCurrentDiagnosticBundle() {
  const targetUri = settings.exportTargetUri.trim();
  if (appInfo.value.platform === "android" && !targetUri) {
    showToast("请先选择 Android SAF 导出目录", "error");
    return;
  ...
  try {
    ...
    appendLog("bridge.export_diagnostic_bundle", result);
    if (!result.ok) {
...
async function reconcileTaskData(taskId = task.taskId, options: { switchToResultsOnData?: boolean } = {}) {
  ...
  if (options.switchToResultsOnData && resultRows.value.length > 0) {
    void navigateTo({ view: "results" });
  }
}

async function reconcileTaskDataSafely(taskId: string, options: { switchToResultsOnData?: boolean }, source: string) {
  try {
    await reconcileTaskData(taskId, options);
  } catch (error) {
    const message = frontendErrorMessage(error);
    appendLog(`${source}.failed`, message);
    reportFrontendRuntimeErrorSilently(source, error, {
      task_id: taskId,
    });
    showToast("结果刷新失败，可手动刷新", "error");
  }
}
...
function applyProbeEvent(event: ProbeEventEnvelope) {
  ...
  if (event.event === "probe.partial_export") {
    ...
    void reconcileTaskDataSafely(
      task.taskId || incomingTaskId,
      {
        switchToResultsOnData: appInfo.value.platform === "android",
      },
      "probe.partial_export.results_refresh",
    );
  }
  ...
  if (event.event === "probe.completed") {
    ...
    void reconcileTaskDataSafely(
      ...
      "probe.completed.results_refresh",
    );
...
watch(selectedView, (view, previousView) => {
  if (previousView === "results" && view !== "results") {
...
onMounted(async () => {
  ...
  window.addEventListener("error", handleFrontendWindowError);
  window.addEventListener("unhandledrejection", handleFrontendUnhandledRejection);
...
onBeforeUnmount(() => {
  ...
  window.removeEventListener("error", handleFrontendWindowError);
  window.removeEventListener("unhandledrejection", handleFrontendUnhandledRejection);
...
<template>
  <DesktopShell :route-title="routeTitles[selectedView]" :selected-view="selectedView" :views="views" @change-view="changeSingleView">
    <DashboardView
    ...
    <SettingsView
      ...
      @export-diagnostic-bundle="exportCurrentDiagnosticBundle"
  ...
  <MobileShell :route-title="routeTitles[selectedView]" :selected-view="selectedView" :views="views" @change-view="changeSingleView">
    <DashboardView
      v-if="selectedView === 'dashboard'"
      :activity-feed="activityFeed"
      :can-pause-task="canPauseTask"
