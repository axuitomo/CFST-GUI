...
bash "$ROOT_DIR/scripts/checks/doctor.sh" --android "${doctor_args[@]}"
...
if ((run_device_smoke)); then
  ...
  bash "$ROOT_DIR/scripts/checks/check-android-device-smoke.sh" "$device_smoke_apk"
else
  cfst_warn "Android device smoke flow skipped; run scripts/checks/android-doctor.sh --device-smoke after connecting a device or AVD"
fi
