...
usage() {
  cat <<'EOF'
usage: scripts/checks/doctor.sh [--strict] [--android]
...
if [[ "$node_version" != "v26.7.0" ]]; then
  printf 'mismatch %-18s required Node.js 26.7.0, found %s\n' "node" "${node_version:-unknown}" >&2
  required_missing=$((required_missing + 1))
fi
