...
usage() {
  cat <<'EOF'
usage: scripts/dev/dev-reset.sh [--apply] [--dry-run] [--frontend-dist] [--skip-check]
...
Options:
  ...
  --skip-check     Skip scripts/checks/check.sh after rebuilding the environment.
EOF
}
...
if ((apply == 0)); then
  ...
  bash "$ROOT_DIR/scripts/dev/clean.sh" "${clean_args[@]}"
  exit 0
fi
