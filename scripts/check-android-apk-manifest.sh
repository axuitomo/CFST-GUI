...
dump_resource_xmltree() {
  ...
  if [[ "$resource_path" == "res/xml/file_paths.xml" ]]; then
    ...
    if grep -Fq '<files-path name="update_downloads" path="update_downloads/" />' "$source_path" &&
      ! grep -Eq '<(root-path|external-path|external-files-path|cache-path|external-cache-path)([[:space:]>])' "$source_path" &&
...
for apk_path in "$@"; do
  ...
  disallowed_path_count="$(grep -Ec 'E: (root-path|external-path|external-files-path|cache-path|external-cache-path)' <<<"$file_paths" || true)"
  if [[ "$files_path_count" != "1" || "$disallowed_path_count" != "0" ]]; then
    echo "Android APK manifest check failed: FileProvider must expose exactly one private files-path and no broad/cache/external paths in $apk_path" >&2
    exit 1
  fi
