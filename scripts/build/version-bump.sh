...
usage() {
  cat <<'EOF'
usage: scripts/build/version-bump.sh <version> [--android-code <code>] [--apply] [--no-release-notes]

Updates release defaults in scripts, workflows, Android Gradle, and docs.
Without --apply, prints the planned changes only.

Examples:
  scripts/build/version-bump.sh 1.7.4
  scripts/build/version-bump.sh 1.8.0 --android-code 10800 --apply
EOF
}
...
if [[ -z "$old_version" ]]; then
  printf 'failed to parse current default version from scripts/build/build-release.sh\n' >&2
  exit 1
fi
if [[ -z "$old_code" ]]; then
