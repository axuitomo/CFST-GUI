...
阶段 1 TCP 默认发包 4 次并跳过首包统计，只有丢包率不超过 15% 的 IP 才会进入后续阶段；当前结果页和 CSV 中的延迟均为 TCP 平均延迟。追踪探测默认并发为 6，最高可设为 20；文件测速固定串行执行，单 IP 内部默认使用 4 个 HTTP Range GET 分片聚合测速，服务端不支持 Range 时回退完整流式 GET。文件测速遇到短文件完成、EOF 或临时断流时会在该时长内自动续连同一 IP，并累计预热后的有效测量窗口。

高级参数包括 TCP并发线程、追踪候选上限、单 IP 下载测速时间、GET 分片并发、下载协议、下载缓冲、端口、文件测速URL、追踪 URL、User-Agent、Host Header、SNI、追踪有效状态码、地区码过滤、调试抓包开关和目标等。旧配置中的下载数量字段仍保留兼容，但不再限制文件测速数量。
...
- `CFST-GUI/cfip-log.txt`：调试日志，默认 JSONL，也可在设置页选择自由格式文本
