...
CFST-GUI 是一个基于 Wails + Vue + Capacitor 的 Cloudflare/CDN IP 测速工具，提供可视化任务面板、输入源管理、结果导出、配置同步和 DNS 推送工作台。
...
- `profiles.json` 管理探测配置档案，`source-profiles.json` 管理输入源档案
...
默认配置根目录由 Go 的 `os.UserConfigDir()` 决定，并追加 `CFST-GUI` 子目录。用户也可以通过界面选择自定义储存目录，或使用 `CFST_GUI_PORTABLE_ROOT` / `portable.json` 启用便携数据目录。

主要文件和目录：

- `storage.json`：储存目录 bootstrap，记录自定义储存位置和初始化状态
- `desktop-config.json`：桌面 GUI / WebUI 当前配置快照
- `mobile-config.json`：Android 当前配置快照，结构与桌面快照同构
- `config.json`：兼容旧桥接结构的配置文件
- `profiles.json`：探测配置档案，包含 `items[].config_snapshot`
- `source-profiles.json`：输入源档案，包含 `items[].sources`
