...
- 普通测速按输入源预处理、TCP 延迟测速、追踪探测、文件测速和结果导出顺序执行
- 输入源可从当前配置中启用多组来源，任务看板会展示 TCP 延迟测速、追踪测试、下载测速三个阶段
...
Cloudflare DNS 推送能力保留在后台链路：定时任务 DNS 推送，以及“测速后自动推送列表”中的 Cloudflare 勾选项会真实创建、更新或删除 DNS 记录。推送会复用 Cloudflare 配置、共享上传策略、Cloudflare Top N 和分流规则；IPv4 写入 A，IPv6 写入 AAAA。执行前请确认 API Token、Zone ID、记录名称和分流规则正确，避免覆盖生产记录。
...
| v1.8.6 发布说明与资产清单 | [docs/release-notes/v1.8.6.md](docs/release-notes/v1.8.6.md) |
...
GitHub Actions 的发行流水线位于 `.github/workflows/release.yml`，由 `v*` tag 或手动触发；正式 Release 成功后会同步调用 Container Package workflow 发布 GHCR 多架构镜像。Android Release 签名需要配置这些 Secrets：`CFST_ANDROID_KEYSTORE_BASE64`、`CFST_ANDROID_KEYSTORE_PASSWORD`、`CFST_ANDROID_KEY_ALIAS`、`CFST_ANDROID_KEY_PASSWORD`。Windows `exe` 安装器签名需要 `CFST_WINDOWS_SIGNING_CERT_BASE64` 和 `CFST_WINDOWS_SIGNING_PASSWORD`，workflow 会在 Windows runner 上安装 NSIS 并生成经典安装包；Windows 安装器负责 WebView2 bootstrapper，Wails 资源版本由 release 脚本按 `CFST_VERSION` 临时同步。
...
bash scripts/version-bump.sh 1.8.6
