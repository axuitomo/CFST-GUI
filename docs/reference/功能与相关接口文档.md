...
配置目录由 Go 的 `os.UserConfigDir()` 决定，并追加 `CFST-GUI` 子目录。若设置 `CFST_GUI_PORTABLE_ROOT`，实际数据目录为 `${CFST_GUI_PORTABLE_ROOT}/data`；若可执行文件同目录存在 `portable.json`，则使用 `<exe-dir>/data`。更完整的字段说明见 [配置详解](../guide/configuration.md)。
