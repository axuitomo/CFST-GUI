...
1. 前端提交 `RunProbe` 后，Capacitor plugin 会先返回 accepted 响应。
2. `ProbeForegroundService` 在前台服务中继续执行真实长任务，并通过同一条 `desktop:probe` 事件流更新系统通知和前端状态。
...
7. Android 默认启用通知栏保活：应用启动后会在通知权限允许时启动独立的 `AndroidKeepAliveForegroundService`，常驻低优先级通知栏；用户可在“异常保护”中关闭，开关只保存在 Android 原生 `SharedPreferences`，不会进入配置、WebDAV 或导入导出链路。
8. Android 自动调度由 `SchedulerWorker` 基于 WorkManager 注册下一次运行；触发后会拉起 `ProbeForegroundService.startScheduledIntent()`，再由 Go 侧读取保存配置并执行单次测速。测速完成后仍会按 `auto_dns_push` 和 `auto_github_export` 配置继续 DNS 推送和 GitHub 导出。受系统省电、厂商后台策略和 Doze 影响，实际触发时间可能晚于配置时间。
...
- Android 调度固定为单次测速模式；旧配置中的 `scheduler.run_mode` 会归一为 `probe`。DNS 推送和 GitHub 导出仍可作为测速后的后续动作配置。
- Android 通知栏保活是 best-effort 机制：它不执行网络轮询、不持有 wake lock，也不会绕过用户强停、系统省电或厂商后台限制；Android 13+ 需要通知权限才能显示常驻通知并启动保活服务。
