...
8. Android 自动调度由 `SchedulerWorker` 基于 WorkManager 注册下一次运行；触发后会拉起 `ProbeForegroundService.startScheduledIntent()`，再由 Go 侧 `runScheduledProbe` 读取保存配置并执行单次测速。Android 调度不会执行工作流；测速完成后仍会按 `auto_dns_push` 和 `auto_github_export` 配置继续 DNS 推送和 GitHub 导出。受系统省电、厂商后台策略和 Doze 影响，实际触发时间可能晚于配置时间。
