...
Activity 使用 `adjustResize`；前端通过 `visualViewport` 同步可视高度、滚动聚焦输入框，并在软键盘打开时隐藏底部导航，避免输入框被遮挡。Android viewport 状态只在可视高度、键盘 inset 或键盘开闭状态真实变化时写入 DOM，避免点击输入框时重复改写 CSS 变量导致 WebView 重排闪烁。
...
- `scripts/android-doctor.sh` 和 `scripts/release-preflight.sh` 会阻塞隐藏 Android 状态栏/系统栏的改动，并检查 Activity 是否保留 edge-to-edge、system bars 可见和 display cutout 短边布局。
