...
MICS 抽样使用 TCP 上的 HTTPS 探测，与后续 `stage1_tcp` 的传输语义保持一致；同一 IP 的多轮请求结束后会立即关闭空闲连接，避免大量抽样连接占用本机临时端口并影响第一阶段。若 TCP 阶段没有可用候选，流程会在 `stage1_tcp` 明确结束，不再伪启动追踪和文件测速；若追踪阶段没有候选，则在 `stage2_trace` 结束且不启动文件测速。
...
| `download_count` 或 `stage_limits.stage3` | `TestCount` / `Stage3Limit` | `download_count` 仅兼容读取；阶段 2 结果按 TCP RTT 升序后，`stage_limits.stage3` 限制进入文件测速的候选数 |
