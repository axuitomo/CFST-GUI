...
本文档基于当前仓库源码整理 CFST-GUI 的功能、运行链路和接口边界。当前项目是一个基于 Wails v2.12.0 + Vue 的桌面端 Cloudflare/CDN IP 测速工具，后端 Go 目标版本为 1.26.6。
...
| Go 工具链 | `go.mod` 固定 `go 1.26.6`；同版本 `toolchain` 行会被 `go mod tidy` 视为冗余并移除 |
