...
- `pushDnsRecords()` 保留给定时任务和测速后自动推送；它会将输入 IP 归一化并按地址族分组，IPv4 覆盖同步 A 记录，IPv6 覆盖同步 AAAA 记录。
...
| Cloudflare DNS | `listDnsRecords()`、`pushDnsRecords()` | `ListCloudflareDNSRecords`、`PushCloudflareDNSRecords` | 同名方法 | `listDnsRecords` 供 DNS 读取页读取 Zone/记录名；`pushDnsRecords` 供定时任务和测速后自动推送覆盖推送记录 |
...
| `run_mode` | 定时运行模式；当前仅执行 `probe`，旧 `pipeline` 配置读取时会降级为普通测速 |
...
当前产品通过 GUI/WebUI/Android 的 DNS 读取页直接调用 Cloudflare DNS 列表 API，不再维护外置示例脚本入口。DNS 读取页只执行查询；创建、更新和删除由定时任务 DNS 推送和测速后自动推送中的 Cloudflare 项触发。
...
| Cloudflare DNS | DNS 页可读取线上记录；定时任务和测速后自动推送可覆盖推送匹配记录 |
...
| DNS 推送 | DNS 读取页不修改线上记录；定时任务和测速后自动推送会真实调用 Cloudflare API 创建、更新和删除匹配记录 |
...
CFST-GUI 当前的核心产品形态是 Wails 桌面 GUI、Linux WebUI 和 Android 移动端桥接。对外用户接口主要是图形界面；程序级接口主要是 Wails/Capacitor 绑定方法、WebUI `/api/*`、前端 bridge 函数和 `desktop:probe` 事件。DNS 读取页已接入 Cloudflare 官方 API 且不修改线上记录；Cloudflare 推送由定时任务或测速后自动推送链路触发。
