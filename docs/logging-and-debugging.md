...
`SettingsView.vue` 的“诊断与日志”折叠区按三层组织：

- 日常排障面板：`导出诊断包`、`导出调试日志`、`查看/打开日志目录`。
- 基础运行日志开关：`settings.loggingEnabled`，附带一句当前策略摘要。
- 临时调试开关：`settings.probeDebug`，附带一句当前调试摘要。

高级设置默认收起：

- `运行日志高级设置` 里放 `settings.loggingLevel`、`settings.loggingRetentionDays`、`settings.loggingMonitorEnabled` 和 `settings.loggingDurability` 的说明。
- `调试高级设置` 里放 `settings.probeDebugLogVerbosity`、`settings.probeDebugCaptureEnabled`、`settings.probeDebugCaptureAddress`。
- `高级请求覆盖` 里放 `settings.probeUserAgent`、`settings.probeHostHeader`、`settings.probeSNI`、`settings.probeRequestHeaders`。

平台文案在界面层区分：

- Android 把日志目录动作弱化为“查看日志位置”。
- Docker WebUI 把日志目录动作改成“查看服务端路径”。
- 桌面端保留“打开日志目录”。
...
当前日志和调试 UI 不是“不能用”，但对日常用户偏复杂，主要问题是把三类完全不同的事塞进了同一块区域：

- 日常操作：导出诊断包、导出调试日志、打开日志目录。
- 长期配置：基础运行日志、等级、保留天数、监控开关。
- 排障配置：临时调试、记录粒度、抓包监听地址。

这会让用户很难在第一眼区分“我要看日志”、“我要改日志策略”还是“我要临时抓问题”。对维护者来说信息足够，但对普通用户来说认知负担偏高。

### 设计目标

1. 默认态只保留最常用的 2 到 3 个动作。
2. 把“日常操作”和“长期配置”拆开。
3. 把“排障配置”默认藏到二级展开里。
4. WebUI、桌面端、Android 的差异在界面上直接显示，不让用户猜。
5. 不改后端数据模型，优先通过界面层收敛复杂度。

### 推荐方案

这是我建议的方案，属于“中等改造”，比纯改文案更有效，但不会把界面重做成两套系统。

| 区域 | 默认显示 | 放到二级/高级 | 平台差异 |
| --- | --- | --- | --- |
| 日常排障 | `导出诊断包` 作为主按钮，`导出调试日志` 作为次按钮 | `打开日志目录` 进入更多操作 | WebUI 改成“查看服务端日志路径”，Android 改成只读提示或导出入口 |
| 基础运行日志 | 开关 + 一句简短状态说明 | `日志等级`、`保留天数`、`运行监控` 放进高级详情 | 对所有平台一致 |
| 临时调试 | 仅保留 `临时开启调试` 开关 | `记录粒度`、`抓包监听地址` 放进调试高级详情 | 只在桌面 / WebUI 可完整呈现，Android 视实现能力降级 |
| 兼容信息 | 当前日志目录和最近一次导出结果 | 具体文件路径和原始 code | 按平台显示不同文案 |

### 交互改法

1. 把当前“日志”和“调试”分成两个层级更清楚的块，而不是一个大折叠里顺着往下堆。
2. 把 `导出诊断包` 提升为主动作，`导出调试日志` 和 `打开日志目录` 放到次级动作。
3. `日志等级`、`保留天数`、`运行监控` 统一放进“高级设置”。
4. `记录粒度`、`抓包监听地址` 默认收起，只在 `临时开启调试` 打开后再显示。
5. 对 WebUI / Android 不成立的动作，不要只靠失败提示，而是直接弱化或隐藏。
6. `分级同步` 这种当前总是固定值的展示，改成说明文案或直接合并进帮助文本，别占一行像可调项。
...
1. **保守版**：只改文案和分组，不动结构。
2. **推荐版**：保留现有功能，重排布局并做渐进披露。
3. **激进版**：把日志页做成“诊断中心”，默认只显示导出和状态，所有配置进抽屉。

我推荐第二种。第一种收效有限，第三种会让改动和验证成本明显上升。

### 实施顺序

1. 先改信息架构，不碰后端。
2. 再改文案，把“临时开启调试”“记录粒度”“分级同步”这些词收紧。
3. 接着做平台适配，WebUI 和 Android 的不可用动作改成明确的替代文案。
4. 最后做视觉收口，让按钮层级、说明文案和高级入口一致。

### 验收标准

- 普通用户能在 5 秒内看懂：这是“导出日志”和“改日志策略”两类不同操作。
- 默认界面里不会同时暴露过多开关。
- 高级排障项不影响日常浏览。
- WebUI 和 Android 上不会出现明显“点了但没意义”的动作。
