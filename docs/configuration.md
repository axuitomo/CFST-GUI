...
| 字段 | 默认值 | 说明 |
| --- | --- | --- |
| `cloudflare_enabled` | `false` | 手动测速完成后，自动执行 Cloudflare 推送。 |
| `github_enabled` | `false` | 手动测速完成后，自动执行 GitHub 导出。 |

`post_probe_push` 不叠加到定时任务；scheduler 会显式禁用这条自动推送入口，避免和原有调度推送重复。
...
| `run_mode` | `probe` | 调度运行模式；当前仅写入和执行 `probe`。旧配置中的 `pipeline` 会在读取时降级为普通测速。 |
...
- 旧 `pipeline_template_id`、`pipeline_target_selector` 或 `run_mode = pipeline` 只用于兼容识别；保存当前配置时不会写回这些字段。
- `auto_dns_push = false` 会在本次调度中跳过 DNS 推送。
- `auto_github_export` 控制定时测速完成后的 GitHub 导出。
- Android 使用系统 WorkManager 注册下一次自动调度，固定执行 `probe` 模式；`auto_dns_push` 和 `auto_github_export` 仍控制测速完成后的 DNS 推送和 GitHub 导出。触发时间可能受系统省电和厂商后台策略影响，建议在“异常保护”中申请电池优化豁免并加入厂商后台白名单。
