...
| 理解日志、调试、诊断包、运行时诊断和 UI 对接 | [日志与调试机制说明](./logging-and-debugging.md) |
| 理解 Android 架构、SAF 文件访问、构建输出和桥接机制 | [Android Mobile Architecture](./android-mobile.md) |
| 查看统一上传筛选、Cloudflare/GitHub 结果上传设计 | [上传自动化设计](./upload-workflow-design.md) |
| 理解输入源获取链路、跨端差异和加速计划 | [输入源获取机制与加速计划](./source-fetching-and-acceleration.md) |
| 查看功能链路、Wails/WebUI/Android API、事件和源码定位 | [功能与相关接口文档](./功能与相关接口文档.md) |
| 查看 v1.8.6 变更摘要、验证命令和发行资产 | [v1.8.6 发布说明](./release-notes/v1.8.6.md) |
...
`docs/logging-and-debugging.md` 说明日志目录、`cfip-log.txt`、运行日志、monitor 心跳、诊断包、运行时诊断、三端接口和设置页/Dashboard 对接。

`docs/android-mobile.md` 说明 Android Capacitor + gomobile 架构、SAF 文件选择、APK 构建输出、在线更新和移动端桥接注意事项。

`docs/upload-workflow-design.md` 说明当前统一上传筛选、Cloudflare/GitHub 目标 Top N、定时任务、测速后自动推送、兼容字段和后续扩展方向。

`docs/source-fetching-and-acceleration.md` 说明输入源获取链路、任务中的作用、桌面端与 Android 差异、当前瓶颈和后续加速计划。

`docs/功能与相关接口文档.md` 说明功能链路、三端 bridge 能力矩阵、WebUI `/api/*`、`desktop:probe` 事件、配置归档、WebDAV、Cloudflare DNS 和源码定位。

`docs/release-notes/v1.8.6.md` 说明 v1.8.6 的移动端前台服务生命周期、输入源缓存异常处理、任务状态修复、验证命令和发行资产。
