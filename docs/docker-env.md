...
| `CFST_ANDROID_VERSION_CODE` | 可选 | Android `versionCode`；默认 `10701`。 |
| `CFST_VERSION` | 可选 | Android `versionName`；默认 `1.7.1`，前缀 `v` 会被去掉。 |
...
该工作流只有手动触发入口，输入 `version` 默认 `1.7.1`。它会先分别运行 `scripts/build-release.sh linux-amd64` 与 `scripts/build-release.sh linux-arm64` 生成 Docker context，再用 Docker Buildx 合并发布单一多架构 tag，覆盖 `linux/amd64` 与 `linux/arm64`。
