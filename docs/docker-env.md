...
| 变量 | 默认值 | 使用位置 | 说明 |
| --- | --- | --- | --- |
| `CFST_VERSION` | `1.7.8` | `scripts/build-release.sh`、Android Gradle | 发行版本号；脚本会写入 Go `github.com/axuitomo/CFST-GUI/internal/app.version`。 |
...
| `CFST_ANDROID_VERSION_CODE` | 可选 | Android `versionCode`；默认 `10708`。 |
| `CFST_VERSION` | 可选 | Android `versionName`；默认 `1.7.8`，前缀 `v` 会被去掉。 |
...
该工作流只有手动触发入口，输入 `version` 默认 `1.7.8`。它会先分别运行 `scripts/build-release.sh linux-amd64` 与 `scripts/build-release.sh linux-arm64` 生成 Docker context，再用 Docker Buildx 合并发布单一多架构 tag，覆盖 `linux/amd64` 与 `linux/arm64`。版本 tag 是固定引用，`latest` 是便捷滚动标签。
