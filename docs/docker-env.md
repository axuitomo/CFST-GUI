...
| 变量 | 默认值 | 使用位置 | 说明 |
| --- | --- | --- | --- |
| `CFST_VERSION` | `1.9.0` | `scripts/build-release.sh`、Android Gradle | 发行版本号；脚本会写入 Go `github.com/axuitomo/CFST-GUI/internal/app.version`。 |
...
| `CFST_ANDROID_VERSION_CODE` | 可选 | Android `versionCode`；默认 `10900`。 |
| `CFST_VERSION` | 可选 | Android `versionName`；默认 `1.9.0`，前缀 `v` 会被去掉。 |
