...
| 变量 | 默认值 | 使用位置 | 说明 |
| --- | --- | --- | --- |
| `CFST_VERSION` | `1.4` | `scripts/build-release.sh`、Android Gradle | 发行版本号；脚本会写入 Go `main.version`。 |
...
| `CFST_ANDROID_VERSION_CODE` | 可选 | Android `versionCode`；默认 `10400`。 |
| `CFST_VERSION` | 可选 | Android `versionName`；默认 `1.4`，前缀 `v` 会被去掉。 |
...
该工作流只有手动触发入口，输入 `version` 默认 `1.4`。它会先运行 `scripts/build-release.sh linux` 生成 Docker context，再用 Docker Buildx 推送 `linux/amd64` 镜像。
