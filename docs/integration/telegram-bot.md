...
- **通用 Webhook**：支持多个地址，每行一个，并可填写 JSON 格式自定义请求头。默认不跟随系统代理，也可以单独切换为跟随系统代理。
- **钉钉机器人**：填写机器人地址；启用加签时填写 Secret，程序会自动生成 `timestamp` 和 `sign`。
- **企业微信机器人**：填写机器人地址，每行一个。
- **企业微信应用消息（手机号）**：手机号不是机器人 Webhook 的能力，需要填写企业 ID、应用 AgentID、应用 Secret 和手机号。程序先按手机号查询成员，再通过企业微信应用消息发送。
- **邮件通知**：填写 SMTP 主机、端口、账号、密码、发件人和收件人。
- Windows 桌面端上传结论会使用系统通知；Android 会在需要时申请通知权限，并发送高优先级通知和系统声音。
